...
static int drawstatusbar(int width, int bh, char *text);
...
int
drawstatusbar(int width, int bh, char *stext)
{
	int ret, i, w, x, len;
	short iscode = 0;
	char *text;
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		tw = barw - drawstatusbar(barw, bh, stext);
	}
...
getatomprop(Client *c, Atom prop)
{
	...
	/* workaround for XEmbedInfo (uses different atom type) */
	Atom req = XA_ATOM;
